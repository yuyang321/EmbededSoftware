...
extern void blockdelay_nus(uint32_t nus);
  ...
  /***************************************************************************************************/
	...
	typedef enum _SYS_TASK_LIST_E_{
		...
		HAL_FSRX_EVENT_E = 1<<1,
		HAL_ADCSAMPLE_EVENT_E = 1<<2,
		HAL_GSMTX_EVENT_E = 1<<3,
		HAL_FSTX_EVENT_E = 1<<4,
		HAL_FAULT_EVENT_E = 1<<5,
		SYS_APL_EVENT_E = 1<<6,
		HAL_TIMER_EVENT_E = 1<<7,
	}SYS_TASK_LIST_e;

	typedef enum _HAL_FAULT_Code_E_{
	USART_GSM_FIFO_FULL = 1<<0,
	USART_GSM_FIFO_EMPTY = 1<<1,
	USART_FSENSOR_FIFO_FULL = 1<<2,
	USART_FSENSOR_FIFO_EMPTY = 1<<3,
	}HAL_FAULT_Code_e;
	...
	/***************************************************************************************************/
	//Arithmetic
	#define ADC_FILT_ARG_VAL 				70			//0--100
	extern uint8_t gLPfiltOneOrder(uint16_t *curfiltedvalp,uint16_t *lastfiltedvalp,__IO uint16_t *databuf,uint8_t bufsize,uint8_t filtarg);
	extern uint8_t gLPfiltMidVal(uint16_t *pdata,uint8_t bufsize,uint16_t *pfiltresult);
#endif
